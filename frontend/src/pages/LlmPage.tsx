...
interface LocalLog {
  time: string; model: string; prompt: string; response: string
  elapsed: number; tokens: number; tps: number; status: string
}
...
export default function LlmPage() {
  ...
  const [systemPrompt, setSystemPrompt] = useState<string>('당신은 친근하고 간결하게 답하는 한국어 어시스턴트입니다.')
  ...
  const run = async () => {
    ...
    setError(null); setResponse(''); setStats(null); setRunning(true)

    try {
      const result = await llmStream(url,
      ...
      setStats({ elapsed: result.elapsed, tokens: result.tokens ?? 0, tps: result.tokens_per_sec ?? 0 })
      setLogs((prev) => [{
        time: new Date().toLocaleTimeString('ko-KR', { hour12: false }),
        model: selectedModel, prompt: prompt.slice(0, 60), response: result.text.slice(0, 80),
        elapsed: result.elapsed, tokens: result.tokens ?? 0, tps: result.tokens_per_sec ?? 0,
        status: '✅ 성공',
      }, ...prev].slice(0, 50))
    } catch (e: any) {
      setError(e.message)
      setLogs((prev) => [{
        time: new Date().toLocaleTimeString('ko-KR', { hour12: false }),
        model: selectedModel, prompt: prompt.slice(0, 60), response: '',
        elapsed: 0, tokens: 0, tps: 0, status: `❌ ${e.message.slice(0, 30)}`,
      }, ...prev].slice(0, 50))
    } finally {
      setRunning(false)
    }
  ...
  return (
    <div className="min-h-full">
      <PageHeader shortcut="02 · LLM" title="Language Model"
                  subtitle="Ollama · 텍스트 입력으로 응답 생성" accent="text-llm" />

      <div className="grid grid-cols-1 lg:grid-cols-5 gap-5 px-8 py-6">
        <div className="lg:col-span-3 space-y-5">
          <Section title="System Prompt">
            <textarea value={systemPrompt} onChange={(e) => setSystemPrompt(e.target.value)}
                      rows={2} className="textarea-style" placeholder="(선택) 시스템 지시문" />
          </Section>

          <Section title="User Prompt">
            <textarea value={prompt} onChange={(e) => setPrompt(e.target.value)}
                      rows={4} className="textarea-style" placeholder="질문이나 지시를 입력..." />
            <div className="flex items-center justify-between">
              <div className="text-2xs font-mono text-ink-500">{prompt.length} chars</div>
              <button onClick={run} disabled={running || !status.online || !selectedModel}
          ...
          <Section title="Response" right={
            stats && <div className="text-2xs font-mono text-ink-400 tabular-nums">
            ...
            <div ref={responseRef}
                 className={`p-4 bg-ink-900 border border-ink-800 rounded min-h-[160px] max-h-[400px] overflow-auto
                            ...
                            ${running ? 'scanline' : ''}`}>
              {response || (
                <span className="text-ink-500 text-2xs font-mono uppercase tracking-wider">no response yet</span>
              )}
        ...
        <div className="lg:col-span-2 space-y-5">
          <Section title="Endpoint"><EndpointCard service="llm" /></Section>
          ...
          <Section title="Model" right={
            ...
            {models.length > 0 ? (
                ...
                {models.map((m) => (
                  <button key={m.name} onClick={() => setSelectedModel(m.name)}
                    ...
                    <div className="flex items-baseline justify-between gap-2">
                      ...
                      <span className="text-2xs text-ink-500 tabular-nums shrink-0">{formatBytes(m.size)}</span>
                    </div>
              ...
              <div className="text-2xs font-mono text-ink-500 uppercase tracking-wider">
                {status.online ? 'no models installed' : 'Ollama 서버 연결 대기 중'}
              </div>
            )}
          </Section>
      ...
      <div className="px-8 pb-8">
        <LogsTable logs={logs} onClear={() => setLogs([])} columns={[
