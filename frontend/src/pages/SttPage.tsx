...
import { sttGetSettings, sttSaveSettings, sttTranscribe, sttGetLogs } from '../api/stt'
...
export default function SttPage() {
  ...
  useEffect(() => {
    ...
    sttGetSettings(url).then((d) => {
      setSettings(d.settings); setModels(d.available_models)
      setLangs(d.available_langs); setComputeTypes(d.compute_types)
    }).catch(() => {})
  }, [url, status.online])
  ...
  return (
    <div className="min-h-full">
      <PageHeader shortcut="01 · STT" title="Speech-to-Text"
                  subtitle="faster-whisper · 음성을 텍스트로 변환" accent="text-stt" />
          ...
          <Section title="Input">
            ...
            {inputMode === 'mic' ? (
              ...
              <div className="space-y-2">
                ...
                {file && <div className="text-2xs font-mono text-ink-400">
                  {file.name} · {(file.size / 1024).toFixed(1)} KB
                </div>}
              </div>
            )}

            <button onClick={runTranscribe} disabled={running || !status.online}
            ...
            {result ? (
              <div className="space-y-3 animate-fade-in">
                ...
                <div className="p-4 bg-ink-900 border border-ink-800 rounded">
                  <div className="text-2xs font-mono text-ink-500 uppercase tracking-wider mb-2">Transcript</div>
          ...
          <Section title="Settings"
            ...
            {settings ? (
              <div className="space-y-3">
                <Field label="Model">
                  <select value={settings.model_size}
                ...
                <Field label="Compute Type">
                  <select value={settings.compute_type}
                ...
                <label className="flex items-center gap-2 text-xs font-mono text-ink-300 cursor-pointer">
                  <input type="checkbox" checked={settings.vad_filter}
                ...
                <div className="text-2xs font-mono text-ink-500">
                  ⚠ model / compute / device 변경 시 모델 재로드
                </div>
              </div>
            ) : (
              <div className="text-2xs font-mono text-ink-500 uppercase tracking-wider">
                서버 연결 대기 중 — Roles 페이지에서 STT role 활성화 후 시작
              </div>
            )}
          </Section>
        </div>
