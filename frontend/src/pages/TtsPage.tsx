...
import { ttsGetSettings, ttsSaveSettings, ttsListVoices, ttsGenerate, ttsGetLogs } from '../api/tts'
...
export default function TtsPage() {
  ...
  const runGenerate = async () => {
    ...
    setError(null); setAudioUrl(null); setRunning(true)
    try {
      const { blob, elapsed } = await ttsGenerate(url, {
        text, language: settings?.language, audio_prompt: settings?.audio_prompt,
        normalize: settings?.normalize, exaggeration: settings?.exaggeration,
      ...
      <div className="grid grid-cols-1 lg:grid-cols-5 gap-5 px-8 py-6">
        <div className="lg:col-span-3 space-y-5">
          ...
          <Section title="Output" right={
            lastElapsed != null && <div className="text-2xs font-mono text-ink-400 tabular-nums">
            ...
            {audioUrl ? (
              <div className="space-y-2 animate-fade-in">
                ...
                <div className="flex justify-end">
                  <a href={audioUrl} download={`tts_${Date.now()}.wav`}
        ...
        <div className="lg:col-span-2 space-y-5">
          <Section title="Endpoint"><EndpointCard service="tts" /></Section>

          <Section title="Settings"
                   right={savingMsg && <span className="text-2xs font-mono text-ink-400">{savingMsg}</span>}>
            {settings ? (
              <div className="space-y-3">
                ...
                <Field label="Voice">
                  <select value={settings.audio_prompt}
                          onChange={(e) => updateSetting('audio_prompt', e.target.value)}
                          className="select-style">
                    <option value="">(기본 목소리)</option>
                    {voices.map((v) => <option key={v.path} value={v.path}>{v.name}</option>)}
                  </select>
                </Field>
                ...
                <label className="flex items-center gap-2 text-xs font-mono text-ink-300 cursor-pointer">
                  <input type="checkbox" checked={settings.normalize}
              ...
              <div className="text-2xs font-mono text-ink-500 uppercase tracking-wider">
                서버 연결 대기 중 — Roles 페이지에서 TTS role 활성화 후 시작
              </div>
            )}
          </Section>
        </div>
