...
const ACCENTS: Record<ServiceKey, { border: string; ring: string }> = {
  stt: { border: 'border-stt/30', ring: 'focus:ring-stt/40' },
  llm: { border: 'border-llm/30', ring: 'focus:ring-llm/40' },
  tts: { border: 'border-tts/30', ring: 'focus:ring-tts/40' },
}
...
export default function EndpointCard({ service, hint }: Props) {
    ...
    <div className="space-y-2">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <span className="text-2xs font-mono text-ink-400 uppercase tracking-wider">Endpoint</span>
        </div>
        <span className={`text-2xs font-mono uppercase tracking-wider
                         ${status.online ? 'text-live' : 'text-ink-500'}`}>
      ...
      <div className="text-2xs font-mono text-ink-500">{hint ?? HINTS[service]}</div>
    </div>
  )
}
