...
// 각 서비스의 health raw에서 의미있는 메타 추출
function extractMeta(service: ServiceKey, raw: any): { label: string; value: string }[] {
  ...
  if (service === 'tts') {
    // /health 응답: { settings: {...}, vram, voice_count, ... }
...
function extractVram(service: ServiceKey, raw: any): VramInfo | null {
  ...
  // LLM은 health에 vram 없음
  return null
}

export default function ServerCard({ service, title, subtitle, shortcut, to }: ServerCardProps) {
  ...
  return (
    <div
      className={`relative flex flex-col bg-ink-900/60 border ${accent.border} rounded-lg
    ...
    >
      ...
      <div className="flex items-start justify-between mb-5">
        <div>
          <div className={`text-2xs font-mono uppercase tracking-[0.2em] ${accent.text}`}>
            {shortcut}
          </div>
          <h2 className="mt-1.5 text-2xl font-medium tracking-tight text-ink-100">
            {title}
          </h2>
          <p className="mt-0.5 text-2xs font-mono text-ink-500 uppercase tracking-wider">
            {subtitle}
          </p>
        </div>
      </div>
      ...
      {/* Process control (Launcher) */}
      <div className="mt-4 pt-4 border-t border-ink-800">
        <ServiceControl service={service} accent={service} />
      </div>
