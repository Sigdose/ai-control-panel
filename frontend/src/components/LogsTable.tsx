    ...
    <div className="border border-ink-800 rounded-md bg-ink-900/40">
      <div className="flex items-center justify-between px-3 py-2 border-b border-ink-800">
        ...
        {onClear && logs.length > 0 && (
          <button
            onClick={onClear}
            className="text-2xs font-mono text-ink-500 hover:text-ink-200 uppercase tracking-wider transition-colors"
          >
            Clear
          </button>
        )}
          ...
          <table className="w-full text-xs font-mono">
            ...
            <tbody>
              {logs.map((row, i) => (
                <tr
                  key={i}
                  className="border-b border-ink-850 hover:bg-ink-850/50 cursor-pointer transition-colors"
