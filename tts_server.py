...
# Flask access log 끄기 (polling 노이즈 제거)
logging.getLogger("werkzeug").setLevel(logging.ERROR)

app = Flask(__name__)
CORS(app)  # React 직결 허용
request_logs = []

# 폴링 노이즈 제거: 이 경로들은 요청 로그 안 찍음
...
# ──────────────────────────────────────────────
# / — 가벼운 JSON health (React 패널에서 폴링용)
# ──────────────────────────────────────────────
...
if __name__ == "__main__":
    print("=" * 60)
    print(f"  TTS Server — Chatterbox Multilingual")
    print(f"  Endpoint : http://0.0.0.0:5002")
    print(f"  Dashboard: http://127.0.0.1:5002/panel")
    print(f"  Health   : http://127.0.0.1:5002/  (JSON)")
    print("=" * 60)
